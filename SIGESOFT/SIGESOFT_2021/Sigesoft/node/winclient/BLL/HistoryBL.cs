...
namespace Sigesoft.Node.WinClient.BLL
{
   public class HistoryBL
   {
      ...
      public List<NoxiousHabitsList> GetNoxiousHabitsReport(string pstrPersonId)
      {
       ...
       public List<FamilyMedicalAntecedentsList> GetFamilyMedicalAntecedentsServiceId(string serviceId)
       {
           try
           {
               SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
               var query = (from pmh in dbContext.personmedicalhistory
                            join ser in dbContext.service on pmh.v_PersonId equals ser.v_PersonId
                            join nox in dbContext.noxioushabits on pmh.v_PersonId equals  nox.v_PersonId
