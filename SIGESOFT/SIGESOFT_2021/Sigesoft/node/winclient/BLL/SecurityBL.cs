...
namespace Sigesoft.Node.WinClient.BLL
{
    public class SecurityBL
    {
        ...
        public List<SystemUserList> GetSystemUserPagedAndFiltered(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression)
        {
            ...
            try
            {
                ...
                var query = (from su1 in dbContext.systemuser
                             ...
                             where su1.i_IsDeleted == 0 && (su1.v_UserName.Contains(pstrFilterExpression) || pstrFilterExpression == "")
                             select new SystemUserList
                             {
        ...
        public int AddSystemUserSAMBHS(ref OperationResult pobjOperationResult, SystemUserSAMBHS data, List<string> ClientSession, int personId)
        {
            int SecuentialId = 0;
            try
            {

                using (var cnx = ConnectionHelperSige.GetConnectionTIS)
                {
                    var create = "INSERT INTO systemuser (i_PersonId, i_UsuarioContable, i_RoleId, v_Password, v_UserName, d_InsertDate, i_InsertUserId, i_IsDeleted) " +
                                 "values (" + personId.ToString() + "," + data.i_UsuarioContable.ToString() + "," + data.i_RoleId.ToString() + ", '" + data.v_Password + "', '" + data.v_UserName + "', " +
                                 "'" + DateTime.Now.ToString() + "', '" + ClientSession[2] + "', 0)";
                ...
                using (var cnx = ConnectionHelperSige.GetConnectionTIS)
                {
                    var userQeury = "select i_SystemUserId from systemuser where i_PersonId = " + personId.ToString() + " and i_IsDeleted = 0";
                    int userId = cnx.Query<int>(userQeury).FirstOrDefault();
                    
                    var createUserNode = "INSERT INTO systemusernode (i_SystemUserId, i_NodeId, d_InsertDate, i_InsertUserId, i_IsDeleted) " +
                                 "values (" + userId.ToString() + ", 2, '" + DateTime.Now.ToString() + "', '" + ClientSession[2] + "', 0)";
                    cnx.Execute(createUserNode);
                }
            ...
            catch (Exception ex)
            {
                ...
                LogBL.SaveLog(ClientSession[0], ClientSession[1], ClientSession[2], LogEventType.CREACION,
                    "USUARIO SAMBHS", string.Format("Id={0}", SecuentialId), Success.Failed, pobjOperationResult.ExceptionMessage);
                return 0;
            }
        }
        ...
        public List<ProfessionalCustom> ContratosPorVencer()
        {
            try
            {
                ...
                var list = (from prof in cnx.professional
                            join per in cnx.person on prof.v_PersonId equals per.v_PersonId
                            join dhy in cnx.datahierarchy on new {a = prof.i_ProfessionId.Value, b = 101} equals new {a = dhy.i_ItemId, b = dhy.i_GroupId}
                            where prof.d_ContractUntil >= desde && prof.d_ContractUntil <= hasta && prof.d_CessationDate == null && per.i_IsDeleted == 0
                            select new ProfessionalCustom
                            {
                                v_PersonId = per.v_PersonId,
                                FullName = per.v_FirstLastName + " " + per.v_SecondLastName + ", " + per.v_FirstName,
                                ContratoDesde = prof.d_ContractFrom,
                                ContratoHasta = prof.d_ContractUntil,
        ...
        public List<ProfessionalCustom> CumpleaniosPorSemana()
        {
            try
            {
                SigesoftEntitiesModel cnx = new SigesoftEntitiesModel();
                ...
                DateTime desde = DateTime.Now.Date;
                DateTime hasta = desde.AddDays(8);
                List<ProfessionalCustom> finalList = new List<ProfessionalCustom>();
                var list = (from prof in cnx.professional
                    join per in cnx.person on prof.v_PersonId equals per.v_PersonId
                    join dhy in cnx.datahierarchy on new { a = prof.i_ProfessionId.Value, b = 101 } equals new { a = dhy.i_ItemId, b = dhy.i_GroupId }
                    where prof.d_CessationDate == null && per.d_Birthdate != null && per.i_IsDeleted == 0
                                  
                    select new ProfessionalCustom
                    {
                        BirthDate = per.d_Birthdate,
                        v_PersonId = per.v_PersonId,
                        FullName = per.v_FirstLastName + " " + per.v_SecondLastName + ", " + per.v_FirstName,
                        ProfessionName = dhy.v_Value1,
                    }).ToList();
                foreach (var item in list)
                {
                    ...
                    if (DateTime.Parse(item.BirthDate.Value.Day.ToString() + "/" + item.BirthDate.Value.Month.ToString() + "/" + DateTime.Now.Year.ToString()) >= desde && DateTime.Parse(item.BirthDate.Value.Day.ToString() + "/" + item.BirthDate.Value.Month.ToString() + "/" + DateTime.Now.Year.ToString()) <= hasta)
                    {
                        ProfessionalCustom obj = new ProfessionalCustom();
                        obj.BirthDate = item.BirthDate;
                        obj.v_PersonId = item.v_PersonId;
                        obj.FullName = item.FullName;
                        obj.ProfessionName = item.ProfessionName;
                        finalList.Add(obj);
                    }
                }
                return finalList;
