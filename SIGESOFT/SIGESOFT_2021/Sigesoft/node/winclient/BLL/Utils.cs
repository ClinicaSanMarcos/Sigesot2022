...
namespace Sigesoft.Node.WinClient.BLL
{
    ...
    public static class Utils
    {
        ...
        public static List<KeyValueDTO> GetSystemParameterForCombo_Consultorio(ref OperationResult pobjOperationResult, int pintGroupId, string pstrSortExpression, int idAtencion)
        {
            //mon.IsActive = true;
            ...
            try
            {
                ...
                var query = from a in dbContext.systemparameter
                            where a.i_GroupId == pintGroupId && a.i_ParentParameterId == idAtencion && a.i_IsDeleted == 0
                            select a;
        ...
        public static List<KeyValueDTO> GetRolSAMBHS()
        {
            try
            {
                using (var cnx = ConnectionHelperSige.GetConnectionTIS)
                {
                    var query = "select i_RoleId as Id, v_Name as Value1 from role where i_IsDeleted = 0 and i_RoleId != 1 order by v_Name";
