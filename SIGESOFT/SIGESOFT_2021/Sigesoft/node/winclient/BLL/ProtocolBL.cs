...
namespace Sigesoft.Node.WinClient.BLL
{
    public class ProtocolBL
    {
        ...
        public List<ProtocolList> GetProtocolPagedAndFiltered(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrComponente)
        {
            ...
            try
            {
                ...
                var query = from A in dbContext.protocol
                            ...
                            join M in dbContext.systemparameter on new { a = A.i_Consultorio.Value, b = 361 }
                                equals new { a = M.i_ParameterId, b = M.i_GroupId } into M5_join
                            from M in M5_join.DefaultIfEmpty()
        ...
        public int? GetSystemUserByPersonId(string personId)
        {
            SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
            int? userId = dbContext.systemuser.Where(x => x.v_PersonId == personId).FirstOrDefault().i_SystemUserId;
            return userId;
        }
