...
namespace Sigesoft.Node.WinClient.UI
{
    public partial class frmLogin : Form
    {
        ...
        private void frmLogin_Load(object sender, EventArgs e)
        {
            ...
            PhysicalAdress = GetMacAddress().ToString();
            List<string> licencias = ObtenerLicencias();
            bool license = false;
            foreach (var lic in licencias)
            {
                if (PhysicalAdress == lic)
                {
                    license = true;
                    break;
                }
            ...
            else
            {
                ...
                txtPassword.Enabled = false;
                btnOK.Enabled = false;
                lblLicense.Text = "No cuenta con licencia activa";
        ...
        private List<string> ObtenerLicencias()
        {
            ConexionSigesoft conectasam = new ConexionSigesoft();
            conectasam.opensigesoft();
            var cadena1 = "select * from Account";
            SqlCommand comando = new SqlCommand(cadena1, connection: conectasam.conectarsigesoft);
            SqlDataReader lector = comando.ExecuteReader();
            List<string> Acount = new List<string>();

            while (lector.Read())
            {
                string dato = lector.GetValue(0).ToString();
                Acount.Add(dato);
            }
        ...
        {
            frmLicense frm = new frmLicense(PhysicalAdress);
            frm.ShowDialog();
            Close();
        }
