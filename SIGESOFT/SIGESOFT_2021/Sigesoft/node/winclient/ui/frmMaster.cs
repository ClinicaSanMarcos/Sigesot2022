...
namespace Sigesoft.Node.WinClient.UI
{
    public partial class frmMaster : Form
    {
        ...
        private void frmMaster_Load(object sender, EventArgs e)
        {
            ...
            {
                CargarPopUpContratosPorVencerYCumpleaños();
            }
        }

        private void CargarPopUpContratosPorVencerYCumpleaños()
        {
            
            var listContr = _objSecurityBL.ContratosPorVencer();
            if (listContr == null) return;

            if (listContr.Count != 0 )
            {
                frmContratosVence frmContr = new frmContratosVence(listContr);
                frmContr.ShowDialog();
            }

            var listCumple = _objSecurityBL.CumpleaniosPorSemana();

            if (listCumple.Count != 0)
            {
                frmCumpleanioSemana frmCumple = new frmCumpleanioSemana(listCumple);
                frmCumple.ShowDialog();
            }
        }
        ...
        private void CargarMenus()
        {
            ...
            if (objAuthorizationList != null)
            {
                ...
                Globals.ClientSession.v_RoleName = roleInfo.V_RoleName == null ? "" : roleInfo.V_RoleName;
