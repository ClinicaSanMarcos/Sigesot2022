...
namespace Sigesoft.Node.WinClient.UI.Reports
{
    public partial class frmManagementReports_Async : Form
    {
        ...
        private void btnConsolidadoReportes_Click(object sender, EventArgs e)
        {
            ...
            {
                System.Threading.Tasks.Task.Factory.StartNew(() => _LogicReports.CrearReportesCrystal(_serviceId, _pacientId, Reportes,_customerOrganizationName, _dni, _pacientName, publicar)).Wait();
            ...
            if (rbConAdjuntos.Checked || rbGenerarPublicar.Checked)
            {
                var adj = _LogicReports._filesNameToMerge.FindAll(p => p.Contains(_dni));
                if (adj.Count() > 0)
                {
