...
namespace Sigesoft.Node.WinClient.UI
{
    public partial class frmOffice : Form
    {
        ...
        {
            string dni = grdListaLlamando.Selected.Rows[0].Cells["v_DocNumber"].Value.ToString();
            OperationResult objOperationResult = new OperationResult();
            ServiceBL objServiceBL = new ServiceBL();
            servicecomponentDto objservicecomponentDto = new servicecomponentDto();
            List<ServiceComponentList> ListServiceComponent = new List<ServiceComponentList>();
            ...
            {
                var checkingFinger = new frmCheckingFinger(dni);
            ...
            if (Result == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                int TserviceId = int.Parse(grdLlamandoPaciente.Selected.Rows[0].Cells["i_ServiceId"].Value.ToString());
                ...
                else
                {
                    ...
                    //frm = new Operations.FrmEsoV2(_serviceId, "TRIAJE", "Service", Globals.ClientSession.i_RoleId.Value, Globals.ClientSession.i_CurrentExecutionNodeId, Globals.ClientSession.i_SystemUserId, (int)MasterService.Eso);
                    //frm.ShowDialog();
                    string PersonId = grdLlamandoPaciente.Selected.Rows[0].Cells["v_PersonId"].Value.ToString();
                    this.Enabled = false;
                    ...
                    t.Start();
                    frm = new Operations.frmContainerEso(_serviceId, "TRIAJE", "Service", TserviceId, PersonId);
                    frm.ShowDialog();
                    this.Enabled = true;
                    //frm = new Operations.frmContainerEso(_serviceId, null, "Service", _TserviceId, _personId);
                    //frm.ShowDialog();
